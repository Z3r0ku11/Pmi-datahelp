# Deploy PMO Framework Hub - Production Environment
param(
    [string]$Region = "us-east-1",
    [string]$StackName = "pmo-framework-hub-prod"
)

Write-Host "🚀 Deploying PMO Framework Hub - Production Environment" -ForegroundColor Green
...
aws cloudformation deploy `
    --template-file prod-infrastructure.yaml `
...
$bucketName = aws cloudformation describe-stacks `
    --stack-name $StackName `
    --region $Region `
    --query 'Stacks[0].Outputs[?OutputKey==`ProdPortalBucket`].OutputValue' `
    --output text

$distributionId = aws cloudformation describe-stacks `
    --stack-name $StackName `
    --region $Region `
    --query 'Stacks[0].Outputs[?OutputKey==`ProdCloudFrontDistribution`].OutputValue' `
    --output text

$portalUrl = aws cloudformation describe-stacks `
    --stack-name $StackName `
    --region $Region `
    --query 'Stacks[0].Outputs[?OutputKey==`ProdPortalURL`].OutputValue' `
    --output text

$userPoolId = aws cloudformation describe-stacks `
    --stack-name $StackName `
    --region $Region `
    --query 'Stacks[0].Outputs[?OutputKey==`ProdCognitoUserPoolId`].OutputValue' `
    --output text

$clientId = aws cloudformation describe-stacks `
    --stack-name $StackName `
    --region $Region `
    --query 'Stacks[0].Outputs[?OutputKey==`ProdCognitoClientId`].OutputValue' `
    --output text
...
$deploymentInfo | Out-File -FilePath "prod-deployment-info.json" -Encoding utf8
...
Write-Host "📁 Deployment info saved to: prod-deployment-info.json" -ForegroundColor White